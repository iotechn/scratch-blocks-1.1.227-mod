 ...
 * @private
 */
Blockly.ZoomControls.prototype.ZOOM_SAVE_PATH_ = 'zoom-save.svg';

/**
 * Zoom return icon path.
 * @type {string}
 * @private
 */
Blockly.ZoomControls.prototype.ZOOM_RETURN_PATH_ = 'zoom-return.svg';
 ...
 * @private
 */
Blockly.ZoomControls.prototype.HEIGHT_ = 212;
...
Blockly.ZoomControls.prototype.createDom = function() {
  ...
  this.createZoomSaveSvg_();
  this.createZoomReturnSvg_();
...
/**
 * Create the zoom save icon and its event handler.
 * Fires Blockly.Events.Ui with element 'zoomSave' so addChangeListener receives it.
 * @private
 */
Blockly.ZoomControls.prototype.createZoomSaveSvg_ = function() {
  var ws = this.workspace_;
  var saveSvg = Blockly.utils.createSvgElement(
      'image',
      {
        'width': this.WIDTH_,
        'height': this.WIDTH_,
  ...
  saveSvg.setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href',
      ws.options.pathToMedia + this.ZOOM_SAVE_PATH_);

  Blockly.bindEventWithChecks_(saveSvg, 'mousedown', null, function(e) {
    ws.markFocused();
    var event = new Blockly.Events.Ui(null, 'zoomSave', undefined, undefined);
...
/**
 * Create the zoom return icon and its event handler.
 * Fires Blockly.Events.Ui with element 'zoomReturn' so addChangeListener receives it.
 * @private
 */
Blockly.ZoomControls.prototype.createZoomReturnSvg_ = function() {
  var ws = this.workspace_;
  var returnSvg = Blockly.utils.createSvgElement(
      'image',
      {
        'width': this.WIDTH_,
        'height': this.WIDTH_,
        'y': (this.WIDTH_ * 4) + (this.MARGIN_BETWEEN_ * 4)
      },
      this.svgGroup_
  );
  returnSvg.setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href',
      ws.options.pathToMedia + this.ZOOM_RETURN_PATH_);

  Blockly.bindEventWithChecks_(returnSvg, 'mousedown', null, function(e) {
    ws.markFocused();
    var event = new Blockly.Events.Ui(null, 'zoomReturn', undefined, undefined);
    event.workspaceId = ws.id;
    Blockly.Events.fire(event);
    Blockly.Touch.clearTouchIdentifier();
    e.stopPropagation();
